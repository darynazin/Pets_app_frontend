...
const PetEditForm = ({ pet }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    breed: pet.breed,
    birthDate:
      formatDateForInput(pet.birthDate) || formatDateForInput(new Date()),
    ...
    try {
      ...
      if (imageFile) {
        try {
          ...
          Swal.fire(
            "Image Upload Failed",
            "Failed to upload image. Pet details were not updated.",
            "error"
          );
          setLoading(false);
          return;
      ...
      Swal.fire(
        "Update Failed",
        err.message || "Failed to update pet",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const handleDeletePet = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Delete Failed",
          "Failed to delete pet. Try again later.",
          "error"
        );
      } finally {
        setLoading(false);
